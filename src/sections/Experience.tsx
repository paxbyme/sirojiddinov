    ...
    <section id="experience" className="py-28">
      <Container>
        ...
        <div className="relative mx-auto max-w-2xl">
          <div className="timeline-line absolute top-0 left-5 h-full w-[2px] rounded-full sm:left-7" />

          <div className="flex flex-col gap-10">
            {profile.experience.map((item, index) => (
              <Reveal key={item.id} delay={80 + index * 100}>
                <div className="theme-card relative p-6 pl-16 sm:pl-20">
                  <div className="icon-box absolute top-6 left-1 flex h-10 w-10 items-center justify-center rounded-2xl text-accent-600 shadow-sm sm:left-3 dark:text-accent-400">
                    {item.type === "work" ? <Briefcase size={17} /> : <GraduationCap size={17} />}
                  </div>

                  <div>
                    <div className="mb-1.5 flex flex-wrap items-baseline gap-x-3 gap-y-1">
                      <h3 className="text-base font-semibold text-neutral-900 dark:text-neutral-50">{item.role}</h3>
                    ...
                    <ul className="space-y-2">
                      {item.description.map((desc, i) => (
                        <li key={i} className="flex gap-3 text-sm leading-relaxed text-neutral-600 dark:text-neutral-300">
                          <span className="bullet-dot mt-2 h-1.5 w-1.5 flex-shrink-0 rounded-full" />
                          {desc}
                        </li>
                      ))}
                    </ul>
